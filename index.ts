...
while (true) {
    ...
    let mainFunction: (mwn: Mwn) => Promise<void>;
    try {
        mainFunction = ((await import(`./scripts/${input}.ts?cb=${Date.now()}`)) as { default: (mwn: Mwn) => Promise<void> }).default;
    } catch (error) {
    ...
    try {
        await mainFunction(loadedMwnInstance);
    } catch (error) {
